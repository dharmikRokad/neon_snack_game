...
class AudioManager {
  ...
  /// Toggle all audio mute state (legacy support)
  void toggleMute() {
    final newMuteState = !(_isSfxMuted && _isMusicMuted);
    _isSfxMuted = newMuteState;
    _isMusicMuted = newMuteState;
    if (_isMusicMuted) {
      pauseBackgroundMusic();
    } else {
      resumeBackgroundMusic();
  ...
  /// Get overall mute state (legacy support)
  bool get isMuted => _isSfxMuted && _isMusicMuted;
